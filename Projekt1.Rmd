...
## 1)
### (a)
 is found in A.2 to be $c = \frac{\alpha e}{\alpha + e}$. Thus we have the acceptance probability is given by:
$$
...
### (a)
Finding expressions for $a$, $b_+$ and $b_-$, and taking the logarithm to avoid high values for the boundaries.
$$
a = \sqrt{\sup_x(x^2 f^*(x))} = \sqrt{\sup_x(x^ {(\alpha -1)}e^{-x})}
$$
$$
