...
rejection.sampling.2 <- function(n){
  normalizing_constant <- integrate(f, 0, 1)
  #print(normalizing_constant$value)
  c <- optimize(f, c(0, 1), maximum=TRUE)$objective/normalizing_constant$value
  #log.c <- log(c)
  samples <- c()
  for(i in 1:n){
    repeat{
      ...
      acceptance.prob <- f(theta)/(normalizing_constant$value*c)
...
mu <- monte.carlo.integration()
mu
integration_constant <- integrate(f, lower=0, upper=1)
numerical_integration <- function(integration_constant){
  h <- 1/10
  a <- 0
  b <- 1
  sum_int <- 0
  for (i in 1:(integer((b-a)/h))){
    sum_int <- sum_int + i*h*f(i*h)/integration_constant
  }
  return (sum_int)
}
Expected_value <- numerical_integration(integration_constant)
Expected_value
```